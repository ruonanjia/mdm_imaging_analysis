...
#     np.savetxt(out_motion, regress_data[motion_columns].fillna(0.0).values, '%g')
    ...
    trial_types = list(set(events.vals.values))
    trial_types.sort()# separate trials by reward magnitude
 
    runinfo = Bunch(
        scans=in_file,
        conditions=[domain + '_' + str(trial_type) for trial_type in trial_types],
    ...
    for condition in runinfo.conditions:        
        ...
        event = events[events.vals == int(condition[4:])]
...
cont5 = ('Med_25>5', 'T', cond_names, [-1, 0, 0, 1, 0, 0, 0, 0, 0])
...
# create workflow
wfSPM = Workflow(name="l1spm_resp_reward", base_dir=work_dir)
wfSPM.connect([
...
########################################################################
# Datasink
datasink = Node(nio.DataSink(base_directory=os.path.join(output_dir, 'Sink_resp_reward')),
                                         name="datasink")
