...
#     np.savetxt(out_motion, regress_data[motion_columns].fillna(0.0).values, '%g')
    ...
    for condition in runinfo.conditions:        
        ...
        runinfo.pmod.append(Bunch(
                name = [condition + '_rating'], #name of modulator for each condition
                param = [np.round(event.ratings.values, 3).tolist()], # values of modulator for each condition
...
templates = {'func': os.path.join(data_root, 'sub-{subject_id}', 'ses-1', 'func', 'sub-{subject_id}_ses-1_task-{task_id}_space-MNI152NLin2009cAsym_desc-preproc_bold.nii.gz'),
             ...
             'events': os.path.join('/home/rj299/project/mdm_analysis/output/', 'event_files', 'sub-{subject_id}_task-{task_id}_cond_v5.csv')}
...
cond_names = ['Med_amb', 'Med_ambxMed_amb_rating^1', 'Med_risk', 'Med_riskxMed_risk_rating^1',
              'Mon_amb', 'Mon_ambxMon_amb_rating^1', 'Mon_risk', 'Mon_riskxMon_risk_rating^1']
...
# SV
cont10 = ['Med_Amb_Rating', 'T', cond_names, [0, 1, 0, 0, 0, 0, 0, 0]]
cont11 = ['Med_Risk_Rating', 'T', cond_names, [0, 0, 0, 1, 0, 0, 0, 0]]
cont12 = ['Mon_Amb_Rating', 'T', cond_names, [0, 0, 0, 0, 0, 1, 0, 0]]
cont13 = ['Mon_Risk_Rating', 'T', cond_names, [0, 0, 0, 0, 0, 0, 0, 1]]
...
# create workflow
wfSPM = Workflow(name="l1spm_resp_rating", base_dir=work_dir)
wfSPM.connect([
...
########################################################################
# Datasink
datasink = Node(nio.DataSink(base_directory=os.path.join(output_dir, 'Sink_resp_rating')),
                                         name="datasink")
